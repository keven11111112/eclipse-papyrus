...
while [[ ! "$updateSite" =~ ^(tmpTest|releases/(indigo|juno|kepler|luna|mars|neon|oxygen)/[0-9]+\.[0-9]+\.[0-9]+|milestones/[0-9]+\.[0-9]+/(I[1-7]|M[1-7]|RC[1-9]|SR[1-9]_RC[1-
9])[a-z]?|nightly/(indigo|juno|kepler|luna|mars|neon|oxygen))$ ]]; do
    echo -n "? "
    read updateSite
done
