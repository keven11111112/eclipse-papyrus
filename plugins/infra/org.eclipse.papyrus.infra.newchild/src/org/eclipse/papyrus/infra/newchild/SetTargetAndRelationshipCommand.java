...
public class SetTargetAndRelationshipCommand extends AbstractCommand {
	 ...
	 */
	public SetTargetAndRelationshipCommand(TransactionalEditingDomain ted, String label, IElementEditService provider, EReference reference, EObject container, IElementType et,ITreeSelectorDialog dialog) { //CreationMenu creationMenu){
		super(label);
...
//			ted.getCommandStack().execute(emfCommand);
	...
	private CreateElementRequest buildRequest(EObject element) {
		...
		if (reference == null) {
			buildRequest = new CreateRelationshipRequest(ted, null,container,element,elementType);
		} else {
			buildRequest = new CreateRelationshipRequest(ted, null,container,element,elementType,reference);
		}
		return buildRequest;
	}
