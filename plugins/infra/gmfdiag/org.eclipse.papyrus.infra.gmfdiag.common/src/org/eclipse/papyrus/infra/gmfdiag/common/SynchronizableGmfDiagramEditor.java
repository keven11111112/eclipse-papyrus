...
public class SynchronizableGmfDiagramEditor extends DiagramDocumentEditor implements IRevealSemanticElement, NavigationTarget {
	...
	protected void updateToggleActionState() {
		...
		if(commandService != null) {
			...
			EclipseCommandUtils.updateToggleCommandState(command, wsPreferenceStore.getBoolean(PreferencesConstantsHelper.VIEW_GRID_CONSTANT));

			command = commandService.getCommand(CommandIds.VIEW_RULER_COMMAND);
			EclipseCommandUtils.updateToggleCommandState(command, wsPreferenceStore.getBoolean(PreferencesConstantsHelper.VIEW_RULERS_CONSTANT));

			command = commandService.getCommand(CommandIds.VIEW_PAGE_BREAK_COMMAND);
			EclipseCommandUtils.updateToggleCommandState(command, wsPreferenceStore.getBoolean(PreferencesConstantsHelper.VIEW_PAGE_BREAK_CONSTANT));
