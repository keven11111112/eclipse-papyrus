 ...
 *
 */
public class SourceEndsButton implements NavigationMenuContributor {
	...
	public List<NavigationMenuButton> getButtons(Object fromElement) {
		...
		if (fromElement instanceof DirectedRelationship) {
			try {
				...
				List<Element> sources = ((DirectedRelationship) fromElement).getSources();
				...
				if (!sources.isEmpty()) {
					...
					if (!navigableElements.isEmpty()) {
						buttons.add(new NavigationSubMenuButton("Source Ends...", null, "Show source ends", navigableElements));
					}
				}
