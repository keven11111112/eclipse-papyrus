...
public class LinkEndDataEditHelperAdvice extends AbstractEditHelperAdvice {
	...
	protected ICommand getAfterConfigureCommand(ConfigureRequest request) {
		...
		// 2] check preference
		if (synchronizePin) {
			LinkEndData editedModelElement = (LinkEndData) request.getElementToConfigure();
			if (editedModelElement != null) {
				// 3] call the command for the ReadLinkAction which owned the LinkEndData
	...
	protected ICommand getAfterSetCommand(SetRequest request) {
		...
		if (request.getFeature().equals(UMLPackage.eINSTANCE.getLinkEndData_End())) {
			...
			// 3] check preference
			if (synchronizePin) {
				LinkEndData editedModelElement = (LinkEndData) request.getElementToEdit();
				if (editedModelElement != null) {
					// 4] call the command for the ReadLinkAction which owned the LinkEndData
					IPinUpdaterLinkEndData updater = new LinkEndDataPinUpdater();
					return new PinUpdateLinkEndDataCommand("Update link end data pins", updater, editedModelElement); //$NON-NLS-1$
				}
			}
