...
public class ReadLinkActionEditHelperAdvice extends AbstractEditHelperAdvice {
	...
	protected ICommand getAfterConfigureCommand(ConfigureRequest request) {
		...
		boolean synchronizePin = (prefStore.getString(IAutomatedModelCompletionPreferencesConstants.READ_LINK_ACTION_ACCELERATOR).equals(AutomatedModelCompletionPreferencesInitializer.PIN_SYNCHRONIZATION));
		// 2] check preference
		if (synchronizePin) {
			ReadLinkAction editedModelElement = (ReadLinkAction) request.getElementToConfigure();
			if (editedModelElement != null) {
				// 3] call the command for the ReadLinkAction
				IPinUpdater<ReadLinkAction> updater = PinUpdaterFactory.getInstance().instantiate(editedModelElement);
				return new PinUpdateCommand<ReadLinkAction>("Update read link action pins", updater, editedModelElement); //$NON-NLS-1$
			}
		}
		return null;
	}
	...
	protected ICommand getAfterSetCommand(SetRequest request) {
		...
		if (request.getFeature().equals(UMLPackage.eINSTANCE.getLinkAction_EndData())) {
			// 2] get the preference for ReadLinkAction
			final IPreferenceStore prefStore = Activator.getDefault().getPreferenceStore();
			boolean synchronizePin = (prefStore.getString(IAutomatedModelCompletionPreferencesConstants.READ_LINK_ACTION_ACCELERATOR).equals(AutomatedModelCompletionPreferencesInitializer.PIN_SYNCHRONIZATION));
			// 3] check preference
			if (synchronizePin) {
				ReadLinkAction editedModelElement = (ReadLinkAction) request.getElementToEdit();
				if (editedModelElement != null) {
					// 4] call the command for the ReadLinkAction
					IPinUpdater<ReadLinkAction> updater = PinUpdaterFactory.getInstance().instantiate(editedModelElement);
					return new PinUpdateCommand<ReadLinkAction>("Update read link action pins", updater, editedModelElement); //$NON-NLS-1$
				}
			}
