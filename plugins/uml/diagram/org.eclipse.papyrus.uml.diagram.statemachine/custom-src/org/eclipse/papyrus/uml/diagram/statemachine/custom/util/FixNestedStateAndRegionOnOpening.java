 ...
 * 
 * - bug 401059, i.e. it removes obsolete compartments from states (made with indigo version of Papyrus)
...
public class FixNestedStateAndRegionOnOpening {
	...
	public void fix(Diagram diagram) {
		...
		final String REGION_ID = "" + RegionEditPart.VISUAL_ID;
		...
		while(it.hasNext()) {
			...
			if(SEP_ID.equals(currentType)) {
				...
				if(parentNode.getChildren().size() == 1) {
					...
					// Fix when current location is not the valid location (only possible if parent size is set)
					TransactionalEditingDomain editingDomain = TransactionUtil.getEditingDomain(diagram);
					Command fixCommand = new FixLocationCommand(editingDomain, "Fix region position on opening", regionBounds, newBounds);
					editingDomain.getCommandStack().execute(fixCommand);
				}
