 ...
 *  Fanch Bonnabesse (ALL4TEC) fanch.bonnabesse@alltec.net - Bug 419357, 494758
 *  
 *****************************************************************************/
...
public class CreateOrShowExistingElementHelper {
		...
		if (UMLPackage.eINSTANCE.getAssociation().equals(wantedEClass)) {
			for (final Element current : ((Element) request.getContainer()).getOwnedElements()) {
				if (hasWantedType(current, wantedElementType)) {
					List<Object> sources = new ArrayList<Object>(this.linkMappingHelper.getSource(current));
					List<Object> targets = new ArrayList<Object>(this.linkMappingHelper.getTarget(current));
	 ...
	 * @return
	 */
	protected boolean checkReflexiveAssociation(CreateRelationshipRequest request, Property firstProperty, Property secondProperty) {
