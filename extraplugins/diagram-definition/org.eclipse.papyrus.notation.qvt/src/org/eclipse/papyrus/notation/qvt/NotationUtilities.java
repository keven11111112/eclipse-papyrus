...
public class NotationUtilities {
		...
		for (IExtension extension : extensionPoint.getExtensions()) {
			for (IConfigurationElement element : extension.getConfigurationElements()) {
				if (element.getName().equals("mapping")) {
					String viewId = element.getAttribute("type");
					String viewType = element.getAttribute("humanReadableType");
					viewMappings.put(viewId, viewType);
				} else if (element.getName().equals("diagramMappings")) {
	...
	private void augmentClazzDiagramMapping(Map<String, String> map) {
		addMapping(map, "Name", "1,5066,5002,5005,5008,5011,5020,5023,5026,5161,5029,5032,5035,5037,0,5155,5153,5157,5040,5043,5046,5049,5052,5055,5017,5162,5014,5058,5061,5064");
		addMapping(map, "Body", "5159,5038,5063,5160");
		addMapping(map, "Constraint", "5067");
		addMapping(map, "Conveyed", "6040");
		addMapping(map, "Containment", "4022");
		addMapping(map, "Attachment", "4016,4014,4013,8500,4024,4025");
	}

	private void augmentProfileDiagramMapping(Map<String, String> map) {
		addMapping(map, "Name", "1,1034,5029,1084,1015,5020,1047,5026,5023,5032,5035,2,5058,1046,5014,5062,1056,1050,1010,1059,5055,5061");
		addMapping(map, "Body", "3,5063,1008,5064");
	...
	private void augmentComponentDiagramMapping(Map<String, String> map) {
		addMapping(map, "Name", "5008,5004,5262,5254,5266,5252,5265,5005,5006,5264,5261,5256,5257,0,5259");
		addMapping(map, "Body", "5255,5253,5258,5260");
	...
	private void augmentDeploymentDiagramMapping(Map<String, String> map) {
		addMapping(map, "Name", "7,48,35,2,5,6,8,9,53,50,37,20,45,22,47,24,43,27,29,41,57");
		addMapping(map, "Body", "3,1,55,58");
		addMapping(map, "Attachment", "4005,4008,4009");
	}

	private void augmentUseCaseDiagramMapping(Map<String, String> map) {
		addMapping(map, "Name", "5014,5015,5016,5017,5019,5025,5026,6039,5032,5018,5030,5029,5031,5020,5021,5022,5023,5024,6006,6007");
		addMapping(map, "QualifiedName", "6037,6041,6040");
		addMapping(map, "Stereotype", "6029,6027,6028");
		addMapping(map, "Body", "6042,5027,5028,6043,6044");
	...
	private void augmentActivityDiagramMapping(Map<String, String> map) {
		...
		addMapping(map, "Body", "5130,5131,5132,5133,5134,5135,5136,5137,5138,5190");
	...
	public String getViewType(View view) {
		if (viewMappings == null || diagramMappings == null)
			buildDiagramMappings();
		String type = viewMappings.get(view.getType());
		if (type == null) {
			Diagram diagram = view.getDiagram();
			Map<String, String> childMappings = diagramMappings.get(diagram.getType());
			type = (childMappings != null) ? childMappings.get(view.getType()) : null;
		}
		return type;
	}
	...
	public Bounds getBounds(Node node) {
		if (cache.containsKey(node.getDiagram())) {
			if (((XMIResource) node.eResource()).getID(node).equals("_98YdsNY-EeSq8-GJgot2zQ")) {
				int i = 0;
				i = i + 1;
			}
			...
			if (figure.getParent() instanceof NodeNamedElementFigure) {
				...
				WrappingLabel stereotypeLabel = parent.getStereotypesLabel();
