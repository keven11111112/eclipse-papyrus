...
public class CreateAssociationCompositeDirectedTest extends AbstractCreateAssociationTest {

	/**
	 * The element id corresponding to the non-directed association.
	 */
	private static final String ELEMENT_TYPE_ID = "org.eclipse.papyrus.uml.AssociationCompositeDirected";
	...
	public void testCreation() throws InterruptedException, RollbackException {
		...
		Assert.assertEquals("The first Association member end must not have aggregation kind", AggregationKind.COMPOSITE_LITERAL, memberEnds.get(0).getAggregation());
		Assert.assertEquals("The first Association member end must be not navigable", true, memberEnds.get(0).isNavigable());
		Assert.assertEquals("The first Association member end must be the association", testClass, memberEnds.get(0).eContainer());
		Assert.assertEquals("The first Association member end must have 0 as lower multiplicity", 0, memberEnds.get(0).getLower());
		Assert.assertEquals("The first Association member end must have 1 as upper multiplicity", 1, memberEnds.get(0).getUpper());
