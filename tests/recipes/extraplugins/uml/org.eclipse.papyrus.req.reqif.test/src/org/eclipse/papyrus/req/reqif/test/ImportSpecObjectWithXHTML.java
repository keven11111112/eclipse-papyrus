...
public class ImportSpecObjectWithXHTML extends AbstractReqIFTest {

	@Test
	public void importSpecObjectWithXHTML() {
		//loading
		//now launch a class diagram
		try {
			UserRegistry.instance.setCurrentUser(new SampleUser());
			initModel("ImportReqIFProject", "importSpecObjectWithXHTML", getBundle());
			getReqIFFile("specObjectHTML.reqif");
			...
			RecordingCommand cmd = new RecordingCommand(getTransactionalEditingDomain(), "importReqIF") {
				...
				protected void doExecute() {
					...
					Assert.assertNotNull("The model must contain a requirement 9", generatedPackage.getOwnedMember("11"));
					org.eclipse.uml2.uml.Class XTMLClass = (org.eclipse.uml2.uml.Class)generatedPackage.getOwnedMember("11");
					List<Stereotype> list = XTMLClass.getAppliedStereotypes();
					Assert.assertNotNull("The class must stereotyped by MyRequirementProfile::ReqType6", XTMLClass.getAppliedStereotype("MyRequirementProfile::ReqType6"));
					Stereotype appliedStereotype = XTMLClass.getAppliedStereotype("MyRequirementProfile::ReqType6");
					Assert.assertEquals("The property HTML must be filled", "testXhtml", XTMLClass.getValue(appliedStereotype, "Attribute1"));
				}
			};
	...
	protected String getSourcePath() {
		return "models/import/importSpecObject_XHTML/";
	}
}
