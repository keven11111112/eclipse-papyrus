...
public class ReImportRemoveSpecObject extends AbstractReqIFTest {
		...
		try {
			...
			RecordingCommand cmd = new RecordingCommand(getTransactionalEditingDomain(), "importReqIF") {
				...
				protected void doExecute() {
					...
					Assert.assertEquals(" Number of addedElement must be equals to 5", 5, deleted.size());
					Set<String> names = new HashSet<String>();
					for(Element element : deleted) {
						if(element instanceof NamedElement) {
							names.add(((NamedElement)element).getName());
						}
					}
					Assert.assertTrue("6 must be the name", names.contains("6"));
					Assert.assertTrue("7 must be the name", names.contains("7"));
					Assert.assertTrue("8 must be the name", names.contains("8"));
					Assert.assertTrue("9 must be the name", names.contains("9"));
					Assert.assertTrue("10 must be the name", names.contains("10"));
				}
			};
	...
	protected String getSourcePath() {
		return "models/re-import/removeLines/";
	}
}
